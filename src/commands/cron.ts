 ...
 */
export function cronEditWarnings(current: CronJob['request'], next: { method: CronMethod; headers?: Record<string, string> }): string[] {
  ...
  return [`warning: the stored request is replaced, not merged (header values are write-only and cannot be read back) — these headers are dropped: ${dropped.join(', ')}`]
}
